...
class Settings(BaseSettings):
    ...
    rva_public_ws_url: str = "ws://127.0.0.1:8081/rva/v1/voice"
    ...
    def validate_runtime(self) -> None:
        ...
        if not self.rva_enabled:
            raise ValueError("VOICE_RVA_ENABLED must remain true for the RVA worker")
        if self.rva_enabled:
            ...
            if rva_public_ws_url.path != "/rva/v1/voice" or rva_public_ws_url.query or rva_public_ws_url.fragment:
                raise ValueError("VOICE_RVA_PUBLIC_WS_URL must use the canonical /rva/v1/voice path")
