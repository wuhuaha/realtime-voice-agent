...
- 当前候选 GitHub Actions 的 repository、server、desktop reference、host E2E、Redis integration、native host contracts
  和 ESP-IDF build/size jobs 已通过。
- 公网 Linux Director/Worker/Redis readiness、real-provider desktop canary、ESP32-S3 WSS 和 UDP 双向 Opus 真机闭环已有
  commit-addressable 证据。
...
Server artifact 和 release SBOM，并在最终 artifact 上完成最小 WSS/UDP smoke。
