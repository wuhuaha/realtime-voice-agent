...
本目录定义 `rva/1` 选择 `udp-opus-gcm/1` 后的 canonical byte wire。WSS 始终负责鉴权、
...
| ---: | ---: | --- | --- |
| 0 | 2 | `magic` | ASCII `VA` |
| 2 | 1 | `wire_version` | 固定 `0x01`；其他值 fail closed |
...
这项语义和 header `wire_version=1` 共同构成 v1 profile。实现不得把其他版本 packet 当 v1 接受，也不得用 profile
...
1. 校验 datagram/header length、magic、`wire_version=1`、单一 flag、payload length 和 MTU。
